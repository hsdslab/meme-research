...
def blurtext(filepath):
    with open('./notebooks/feature-matching/unprocessed_files.txt', 'a') as log:
        try:
            ...
            try:
                os.mkdir(os.path.join(full_path,foldername))
            except OSError as error:
                pass
            targetloc = os.path.join(full_path,foldername,filename)

            # print('Writing to: ',targetloc)
            cv2.imwrite(targetloc,orig)
            with open('./notebooks/feature-matching/processed_files.txt', 'a') as log:
                log.write(filename+'\n')
...
def get_checkpoint_index(files_df):
    try:
        with open('./notebooks/feature-matching/processed_files.txt', 'r') as log:
            processed_files = log.readlines()

        # drop empty lines
        processed_files = [x.strip() for x in processed_files if x.strip()]
        # get the last processed file
        last_processed_file = processed_files[-1]

        id = os.path.basename(os.path.splitext(last_processed_file)[0])
        # get the row from files_df with the id id
        row = files_df[files_df['id'] == id]
        # get the index of the row
        checkpoint_index = row.index[0]
...
if __name__ == '__main__':
    ...
    basepath = "D:/"
    save_dir = "SM400_blurred"
    textdetect = './models/feature-matching/frozen_east_text_detection.pb'

    try:
        full_path = os.path.join(basepath, save_dir)
        os.mkdir(full_path)
    except OSError as error:
        pass

    files_df = pd.read_parquet(r'D:\Murgi\code\meme-evaluator\v3\v3_predictions.parquet')
    ...
    start = time.time()
    starting_index = get_checkpoint_index(files_df)
    print('Starting from index:',starting_index)

    r = thread_map(blurtext, files[starting_index:], max_workers=7)
