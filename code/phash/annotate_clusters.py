...
def process_kym_files(kym_phashes_file):
    ...
    with open(kym_phashes_file) as fd:
        for idx, line in enumerate(fd.readlines()):
            ...
            template = os.path.basename(image).split('_')[0]
            ...
            if template not in kym_phashes_by_meme_dic:
                kym_phashes_by_meme_dic[template] = []
            kym_phashes_by_meme_dic[template].append(phash)

            kym_images_dic[phash] = image
            kym_images_dic_reverse[image] = phash
            kym_meme_name[phash] = template
...
def main(options, arguments):
    ...
    kym_phashes_file =  r".\data\phashes\imgflip_annotate_phashes.txt"
    outfile =  "imgflip_clustering_selection.json"
    ...
    cluster_hashes = []
    for cid in tqdm(clusters, total=len(clusters), desc='[i] processing clusters'):
